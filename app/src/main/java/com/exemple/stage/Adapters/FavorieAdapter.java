...
public class FavorieAdapter extends RecyclerView.Adapter<FavorieAdapter.ImageViewHolder2> {
    DatabaseReference databaseReference = FirebaseDatabase.getInstance().getReference("Videos");
    Videos videos;
    private Context mContext;
    private List<Exercice> mUploads;

    public FavorieAdapter(Context context, List<Exercice> uploads) {
        this.mUploads = uploads;
        this.mContext = context;
    }

    @Override
    public FavorieAdapter.ImageViewHolder2 onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(mContext).inflate(R.layout.favorie_list, parent, false);
        return new FavorieAdapter.ImageViewHolder2(view);
    }
